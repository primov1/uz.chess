...
let MatchPublicService = class MatchPublicService {
    async getAll() {
        ...
        return (0, class_transformer_1.plainToInstance)(match_list_public_dto_1.MatchListPublicDto, matches, { excludeExtraneousValues: true });
    }
    async getOne(id) {
        const match = await match_entity_1.Match.findOneBy({ id });
        if (!match)
            throw new common_1.NotFoundException('Match with given id not found');
        return (0, class_transformer_1.plainToInstance)(match_detail_public_dto_1.MatchDetailPublicDto, match, { excludeExtraneousValues: true });
    }
};
