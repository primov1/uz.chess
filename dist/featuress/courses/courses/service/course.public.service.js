...
let CoursePublicService = class CoursePublicService {
    async getAll() {
        ...
        return (0, class_transformer_1.plainToInstance)(course_list_public_dto_1.CourseListPublicDto, items, { excludeExtraneousValues: true });
    }
    async getOne(id) {
        const item = await course_entity_1.Course.findOneBy({ id });
        if (!item)
            throw new common_1.NotFoundException('Course with given id not found');
        return (0, class_transformer_1.plainToInstance)(course_detail_public_dto_1.CourseDetailPublicDto, item, { excludeExtraneousValues: true });
    }
};
